...
public static partial class SpracheParser
{
    ...
    private static readonly Parser<Expression> Expression = Sprache.Parse.Ref(() => InRangeExpression);
    private static readonly Parser<RangeExpression> RangeExpression = Sprache.Parse.Ref(() => RangeExpressionImpl);

    // Expression hierarchy
    private static Parser<Expression> PrimaryExpression =>
        Expression.Contained(Sprache.Parse.Char('('), Sprache.Parse.Char(')'))
    ...
    private static Parser<Expression> ComparisonExpression =>
        PrimaryExpression.Then(left =>
            (Token("==").Return(BinaryOperator.Equals)
                ...
                .Then(op => PrimaryExpression.Select(right => 
                    new BinaryExpression { Left = left, Right = right, Operator = op } as Expression)))
            .Or(Sprache.Parse.Return(left)));

    private static Parser<Expression> AndExpression =>
        ComparisonExpression.Then(left =>
            (Token("&&").Then(_ => ComparisonExpression.Select(right => 
                new BinaryExpression { Left = left, Right = right, Operator = BinaryOperator.And } as Expression)))
            .Or(Sprache.Parse.Return(left)));

    private static Parser<Expression> OrExpression =>
        AndExpression.Then(left =>
            (Token("||").Then(_ => AndExpression.Select(right => 
                new BinaryExpression { Left = left, Right = right, Operator = BinaryOperator.Or } as Expression)))
            .Or(Sprache.Parse.Return(left)));

    private static Parser<Expression> InRangeExpression =>
        OrExpression.Then(left =>
            Token("in").Then(_ => RangeExpression.Select(range => 
                new InRangeExpression { Left = left, Range = range } as Expression))
            .Or(Sprache.Parse.Return(left)));

    // Range expressions
    private static Parser<RangeExpression> RangeExpressionImpl =>
        TimeOfDay.Then(start =>
            Token("..").Then(_ => 
                TimeOfDay.Select(end => 
                    new TimeRangeExpression { Start = start, End = end } as RangeExpression)))
        .Or(PrimaryExpression.Then(start =>
            Token("..").Then(_ => 
