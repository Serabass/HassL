...
public static partial class SpracheParser
{
  ...
  private static readonly Parser<object?> SkipWhitespace = Sprache
    ...
    .Or(BlockComment.Select(_ => (object?)null))
    .Many()
    .Select(_ => (object?)null);

  // Basic tokens
  private static Parser<T> Token<T>(Parser<T> parser) =>
    parser.Contained(SkipWhitespace, SkipWhitespace);
  ...
  private static readonly Parser<string> Identifier = Token(
    Sprache
      .Parse.Letter.Or(Sprache.Parse.Char('_'))
      .Then(c =>
        Sprache
          .Parse.LetterOrDigit.Or(Sprache.Parse.Char('_'))
          .Many()
          .Text()
