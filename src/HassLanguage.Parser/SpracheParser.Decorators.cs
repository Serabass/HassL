...
public static partial class SpracheParser
{
    ...
    private static Parser<Decorator> Decorator =>
        Sprache.Parse.Char('@').Then(_ =>
            Identifier.Then(name =>
                Sprache.Parse.Char('(').Then(_ =>
                    DecoratorArgument.DelimitedBy(Sprache.Parse.Char(',').Contained(SkipWhitespace, SkipWhitespace))
                        .Contained(SkipWhitespace, SkipWhitespace)
                        .Then(args =>
                            Sprache.Parse.Char(')').Return(new Decorator 
                            { 
                                Name = name, 
    ...
    private static Parser<DecoratorArgument> DecoratorArgument =>
        Identifier.Select(id => new IdentifierDecoratorArgument { Value = id } as DecoratorArgument)
