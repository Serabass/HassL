...
public static partial class SpracheParser
{
    ...
    private static Parser<ActionBlock> ActionBlock =>
        Token("{").Then(_ =>
            ActionStatement.Many().Then(statements =>
            ...
            Token("when").Then(_ =>
                ConditionExpression.Then(condition =>
                    ActionBlock.Then(actions =>
                        Sprache.Parse.Return(new WhenClause 
                        { 
    ...
    private static Parser<AutomationDeclaration> AutomationDeclaration =>
        DecoratorList.Optional().Then(decorators =>
            Token("automation").Then(_ =>
                StringLiteral.Then(displayName =>
                    Token("{").Then(_ =>
                        WhenClause.Many().Then(whenClauses =>
                            Token("}").Return(new AutomationDeclaration 
                            { 
