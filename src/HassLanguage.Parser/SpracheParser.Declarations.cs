...
public static partial class SpracheParser
{
    ...
    private static Parser<EntityDeclaration> EntityDeclaration =>
        EntityType.Then(type =>
            Identifier.Then(alias =>
                Token("{").Then(_ =>
                    EntityProperty.Many().Then(props =>
                        Token("}").Return(new EntityDeclaration 
                        { 
                            Type = type, 
                            Alias = alias, 
    ...
    private static Parser<string> EntityType =>
        Token("light").Or(Token("binary_sensor")).Or(Token("sensor"))
            .Or(Token("climate")).Or(Token("switch")).Or(Token("cover"))
            .Or(Identifier);
        ...
        Identifier.Then(key =>
            Token(":").Then(_ =>
                Expression.Then(value =>
                    Token(";").Return((key, value)))));

    private static Parser<List<EntityDeclaration>> EntityList =>
        Token("entities").Then(_ =>
            Token(":").Then(_ =>
                Token("[").Then(_ =>
                    EntityDeclaration.DelimitedBy(Sprache.Parse.Char(',').Contained(SkipWhitespace, SkipWhitespace))
                        .Contained(SkipWhitespace, SkipWhitespace)
                        .Then(entities =>
                            Token("]").Then(_ =>
                                Token(";").Return(entities.ToList()))))));

    // Device declaration
    private static Parser<DeviceDeclaration> DeviceDeclaration =>
        DecoratorList.Optional().Then(decorators =>
            Token("device").Then(_ =>
                StringLiteral.Then(displayName =>
                    Identifier.Then(alias =>
                        Identifier.Optional().Then(type =>
                            Token("{").Then(_ =>
                                EntityList.Optional().Then(entities =>
                                    Token("}").Return(new DeviceDeclaration 
                                    { 
                                        DisplayName = displayName, 
                                        Alias = alias, 
                                        Type = type.IsDefined ? type.Get() : null,
                                        Decorators = decorators.IsDefined ? decorators.Get() : new List<Decorator>(),
                                        Entities = entities.IsDefined ? entities.Get() : new List<EntityDeclaration>()
                                    }))))))));
            ...
            Token("room").Then(_ =>
                StringLiteral.Then(displayName =>
                    Identifier.Then(alias =>
                        Identifier.Optional().Then(type =>
                            Token("{").Then(_ =>
                                DeviceDeclaration.Many().Then(devices =>
                                    Token("}").Return(new RoomDeclaration 
                                    { 
                                        DisplayName = displayName, 
                                        Alias = alias, 
                                        Type = type.IsDefined ? type.Get() : null,
