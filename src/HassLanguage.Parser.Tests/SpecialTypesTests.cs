...
public class SpecialTypesTests
{
    ...
    public void ParseDuration_ShouldParseCorrectly(string input, int expectedValue, DurationUnit expectedUnit)
    {
        // Act
        var result = HassLanguageParser.Parse($"automation \"Test\" {{ when test.value == 5 for {input} {{ do test(); }} }}");
    ...
    public void ParseTimeOfDay_ShouldParseCorrectly(string input, int expectedHour, int expectedMinute)
    {
        // Act
        var result = HassLanguageParser.Parse($"automation \"Test\" {{ when time in {input}..18:00 {{ do test(); }} }}");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("automation \"Test\" { when 2024-01-15T10:30:45 == 2024-01-15T10:30:45 { do test(); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("automation \"Test\" { when test.value == 5 { do test.func({ brightness: 70; color: \"red\"; }); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("automation \"Test\" { when test.value == 5 { do test.func({ a: 1, b: 2, c: 3 }); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("automation \"Test\" { when test.value == 5 { do test.func({ }); } }");
