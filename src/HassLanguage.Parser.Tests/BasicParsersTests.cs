...
public class BasicParsersTests
{
    ...
    {
        // Act
        var result = HassLanguageParser.Parse($"automation \"Test\" {{ when {input} == {expected} {{ do test(); }} }}");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse($"automation \"Test\" {{ when {input} > 0.0 {{ do test(); }} }}");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse($"automation \"Test\" {{ when {input} == {expected.ToString().ToLower()} {{ do test(); }} }}");
