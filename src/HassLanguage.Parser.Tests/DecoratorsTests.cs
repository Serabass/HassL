...
public class DecoratorsTests
{
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("@mode(\"restart\") automation \"Test\" { when test.value == 5 { do test(); } }");
    ...
    public void ParseDecorator_ShouldParseWithIdentifierArgument()
    {
        ...
        result.Automations.Should().HaveCount(1);
        result.Automations[0].Decorators.Should().HaveCount(1);
        result.Automations[0].Decorators[0].Arguments[0].Should().BeOfType<IdentifierDecoratorArgument>();
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("@cooldown(10s) automation \"Test\" { when test.value == 5 { do test(); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("@retry(3) automation \"Test\" { when test.value == 5 { do test(); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("@test(\"arg1\", 10s, 5) automation \"Test\" { when test.value == 5 { do test(); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("@mode(restart) @cooldown(10s) automation \"Test\" { when test.value == 5 { do test(); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("automation \"Test\" { @edge(rising) when test.value == 5 { do test(); } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("home \"TestHome\" test { @priority(high) room \"TestRoom\" room { } }");
    ...
    {
        // Act
        var result = HassLanguageParser.Parse("home \"TestHome\" test { room \"TestRoom\" room { @enabled(true) device \"TestDevice\" device { } } }");
